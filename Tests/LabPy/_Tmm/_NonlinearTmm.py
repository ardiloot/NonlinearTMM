...
class _InhomogeneosWave:
    ...
    def Solve(self, kx: complex, kpS: tuple | None) -> None:
        ...
        if not self.isNonlinear:
            ...
            if layer.pol == "s":
                BFunc = lambda py: -(py / Constants.eps0) * layer.k0**2.0 / (layer.k**2.0 - self.kS**2.0)
                self.ByF = BFunc(self.pyF)
                self.ByB = BFunc(self.pyB) * np.exp(1.0j * self.kSzF * layer.d)
            elif layer.pol == "p":
                BFunc = lambda px, pz, kSz: -layer.omega / (layer.k**2.0 - self.kS**2.0) * (kSz * px - self.kx * pz)
...
class _NonlinearLayer(Core.ParamsBaseClass):
    ...
    def GetFields(self, zs: NDArray, outBackAndForward: bool = False) -> tuple[NDArray, NDArray]:
        ...
        if self.pol == "p":
            ...
            E[:, :, X] = c * (
                ...
                + self.omega * self.iws.px * expKSz
                + self.iwa.By * (self.hw.kz - self.iwa.kSz) * expKAz
                + self.omega * self.iwa.px * expKAz
            )
            E[:, :, Z] = c * (self.hw.kx * Hy + self.omega * self.iws.pz * expKSz + self.omega * self.iwa.pz * expKAz)
        elif self.pol == "s":
    ...
    def GetAbsorbedPower(self) -> float:
        ...
        intValue = (
            np.dot(E0F, np.conj(E0F)) * intValue1
...
class _NonlinearTmm(Core.ParamsBaseClass):
    ...
    def Solve(self, **kwargs: Any) -> tuple:
        ...
        if self.mode == self.MODE_INCIDENT:
            if self.overrideE0 is None:
                ...
                if self.pol == "p":
                    inc = np.sqrt(
                        abs(2.0 * omega * Constants.eps0 / l0.hw.kzF.real * l0.eps.real * self.I0 * cosTh0),
                        dtype=complex,
                    )
                elif self.pol == "s":
    ...
    def __TransferMatrix(self, layerNr: int) -> tuple[NDArray, NDArray]:
        ...
        def TransferMatrixNL(w1: _InhomogeneosWave, w2: _InhomogeneosWave) -> NDArray:
            ...
            if self.pol == "p":
                f3 = -l1.omega * l2.eps * (w1.pxB * exp1B + w1.pxF * exp1F) + l1.omega * l1.eps * (w2.pxB + w2.pxF)
...
class SecondOrderNLTmm(Core.ParamsBaseClass):
    ...
    def Sweep(
        ...
        for i in range(len(paramValues[0])):
            ...
            if enhpos is not None:
                ...
                enhP1 = np.linalg.norm(EP1[0, :]) / (
                    np.linalg.norm(E0P1[0, :]) * np.sqrt(self.tmmP1.layers[0].n(self.wlP1).real)
                )
                ...
                enhP2 = np.linalg.norm(EP2[0, :]) / (
                    np.linalg.norm(E0P2[0, :]) * np.sqrt(self.tmmP2.layers[0].n(self.wlP2).real)
                )
    ...
    def _SolveFundamentalFields(self) -> None:
        ...
        self.tmmP2 = _NonlinearTmm(
            I0=self.I0P2, overrideE0=self.overrideE0P2, wl=self.wlP2, pol=self.polP2, mode=_NonlinearTmm.MODE_INCIDENT
        )
