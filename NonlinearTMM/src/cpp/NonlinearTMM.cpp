...
SweepResultNonlinearTMM::SweepResultNonlinearTMM(int n, int outmask_, int layerNr_, double layerZ_)
    : inc(n), r(n), t(n), Ii(n), Ir(n), It(n), Ia(n), enh(n) {
    outmask = outmask_;
    layerNr = layerNr_;
    layerZ = layerZ_;
...
MatrixXd FieldsZX::GetENorm() const {
    ...
    case Polarization::P_POL:
        res =
            (Ex.array().real().pow(2) + Ex.array().imag().pow(2) + Ez.array().real().pow(2) + Ez.array().imag().pow(2))
                .sqrt();
        break;
    case Polarization::S_POL:
...
Array2cd NonlinearTMM::CalcTransferMatrixNL(int interfaceNr, const InhomogeneousWave& w1, const InhomogeneousWave& w2) {
    ...
    if (pol == Polarization::P_POL) {
        f3 = -l1.omega * l2.eps * (w1.px(iB) * w1.phaseS(1) + w1.px(iF) * w1.phaseS(0)) +
             l1.omega * l1.eps * (w2.px(iB) + w2.px(iF));
...
void NonlinearTMM::SolveInterfaceTransferMatrix(int interfaceNr) {
    ...
    transferMatricesNL[interfaceNr] =
        CalcTransferMatrixNL(interfaceNr, l1.iws, l2.iws) + CalcTransferMatrixNL(interfaceNr, l1.iwa, l2.iwa);
}
...
double NonlinearTMM::CalcVacFuctuationsE0() {
    ...
    double ESqr = (deltaWlSpdc * solidAngleSpdc / deltaThetaSpdc) * (constHbar / (8.0 * constEps0 * std::pow(PI, 4))) *
                  (std::pow(omegaGen, 3) * std::pow(omegaP2, 2) / (pow(constC, 4))) * (constC / omegaP2);
...
std::unique_ptr<SweepResultNonlinearTMM> NonlinearTMM::Sweep(TMMParam param, const Eigen::Map<ArrayXd>& values,
                                                             int outmask, int paramLayer, int layerNr, double layerZ) {
...
std::unique_ptr<FieldsZX> NonlinearTMM::GetFields2D(const Eigen::Map<ArrayXd>& zs, const Eigen::Map<ArrayXd>& xs,
                                                    WaveDirection dir) {
    if (!solved) {
        throw std::runtime_error("NonlinearTMM must be solved first.");
    }
...
std::unique_ptr<FieldsZX> NonlinearTMM::WaveGetFields2D(const Eigen::Map<ArrayXd>& zs, const Eigen::Map<ArrayXd>& xs,
                                                        WaveDirection dir) {
    CheckPrerequisites();
...
std::unique_ptr<WaveSweepResultNonlinearTMM> NonlinearTMM::WaveSweep(TMMParam param, const Eigen::Map<ArrayXd>& values,
                                                                     int outmask, int paramLayer, int layerNr,
                                                                     double layerZ) {
    CheckPrerequisites(param);
    if (layerNr < 0 || layerNr > layers.size()) {
        throw std::invalid_argument("Invalid layer index.");
...
pairdd NonlinearTMM::WaveGetPowerFlows(int layerNr, double x0, double x1, double z) {
    ...
    if (wave.IsCoherent()) {
        ...
        for (int i = 0; i < betas.size(); i++) {
            auto [rFwd, rBwd] =
                IntegrateWavePower(layerNr, pol, wl, epsLayer, Us.row(i), kxs.row(i), kzs.row(i), x0, x1, z, Ly);
            double dkx = GetDifferential(kxs, i);
            res.first += rFwd * dkx;
            res.second += rBwd * dkx;
        }
...
void NonlinearTMM::UpdateSPDCParams(double deltaWlSpdc_, double solidAngleSpdc_, double deltaThetaSpdc_,
                                    double wlP1Spdc_, double betaP1Spdc_) {
...
WaveSweepResultNonlinearTMM::WaveSweepResultNonlinearTMM(int n, int outmask_, int layerNr_, double layerZ_)
    : Pi(n), Pr(n), Pt(n), enh(n), beamArea(n) {
    outmask = outmask_;
    layerNr = layerNr_;
    layerZ = layerZ_;
