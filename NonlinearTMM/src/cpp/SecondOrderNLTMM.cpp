...
SweepResultSecondOrderNLTMM::SweepResultSecondOrderNLTMM(int n, int outmask_, int layerNr_, double layerZ_)
    : P1(n, outmask_, layerNr_, layerZ_), P2(n, outmask_, layerNr_, layerZ_),
      Gen(n, outmask_ & ~(SWEEP_ENH), layerNr_, layerZ_), wlsGen(n), betasGen(n) {
    outmask = outmask_;
}
...
void SecondOrderNLTMM::CalcInhomogeneosWaveParams(int layerNr, Material* material, InhomogeneosWaveParams* kpS,
...
std::unique_ptr<SweepResultSecondOrderNLTMM> SecondOrderNLTMM::Sweep(TMMParam param,
                                                                     const Eigen::Map<ArrayXd>& valuesP1,
                                                                     const Eigen::Map<ArrayXd>& valuesP2, int outmask,
                                                                     int paramLayer, int layerNr, double layerZ) {
...
std::unique_ptr<FieldsZX> SecondOrderNLTMM::WaveGetFields2D(const Eigen::Map<ArrayXd>& zs,
                                                            const Eigen::Map<ArrayXd>& xs, WaveDirection dir) {
    // Do checking
    UpdateGenParams();
    CheckPrerequisites();
...
pairdd SecondOrderNLTMM::WaveGetPowerFlows(int layerNr, double x0, double x1, double z) {
    ...
    if (waveP1->IsCoherent() && waveP2->IsCoherent()) {
        ...
        std::sort(indices.data(), indices.data() + nTot,
                  [&kxsUnsorted](const int a, int b) -> bool { return kxsUnsorted(a) < kxsUnsorted(b); });
...
std::unique_ptr<WaveSweepResultSecondOrderNLTMM>
SecondOrderNLTMM::WaveSweep(TMMParam param, const Eigen::Map<ArrayXd>& valuesP1, const Eigen::Map<ArrayXd>& valuesP2,
